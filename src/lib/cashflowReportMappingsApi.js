// src/lib/cashflowReportMappingsApi.js
...
export async function listMappings({ companyId, standard = null }) {
  if (!companyId) return [];
  let url = `${SUPABASE_URL}/cashflow_report_mappings?select=*&company_id=eq.${companyId}&is_archived=eq.false&order=updated_at.desc`;
...
export async function createMapping({
  ...
  const payload = {
    ...
    cf_tree: cfTree, highlighted_ids: highlightedIds, cf_view_mode: cfViewMode,
    created_by: userId, updated_by: userId,
  };
  const res = await fetch(`${SUPABASE_URL}/cashflow_report_mappings`, {
    method: "POST",
...
export async function updateMapping({
  ...
  const res = await fetch(
    `${SUPABASE_URL}/cashflow_report_mappings?mapping_id=eq.${mappingId}`,
