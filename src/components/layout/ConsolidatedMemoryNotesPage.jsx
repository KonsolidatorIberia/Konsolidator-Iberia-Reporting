...
// ── Addons Genesal (Nota 23) ──────────────────────────────────────
  ...
  related_ops: {
    label: "Partes vinculadas · Operaciones (23.1)",
    columns: [
      { id: "c-group", label: "Otras empresas del grupo",       col_type: "value" },
      { id: "c-key",   label: "Personal clave de la dirección", col_type: "value" },
    ],
    rows: [
      { label: "Ventas de activos corrientes" },
      { label: "Prestación de servicios" },
      { label: "Recepción de servicios" },
      { label: "Ingresos por intereses devengados pero no cobrados" },
      { label: "Dividendos y otros beneficios distribuidos" },
    ],
  },
  // 23.2 Saldos con vinculadas: jerarquía PGC 3 niveles, una columna de importe.
  related_bal: {
    label: "Partes vinculadas · Saldos (23.2)",
    columns: [
      { id: "c-group", label: "Otras empresas del grupo", col_type: "value" },
    ],
    rows: [
      { label: "A) ACTIVO NO CORRIENTE", level: 0, is_subtotal: true },
      { label: "1. Inversiones financieras a largo plazo", level: 1 },
      { label: "b) Créditos", level: 2 },
      { label: "B) ACTIVO CORRIENTE", level: 0, is_subtotal: true },
      { label: "1. Deudores comerciales y otras cuentas a cobrar", level: 1 },
      { label: "b) Clientes por ventas y prestación de servicios a corto plazo", level: 2 },
      { label: "2. Inversiones financieras a corto plazo", level: 1 },
      { label: "b) Créditos", level: 2 },
      { label: "e) Otros activos financieros", level: 2 },
      { label: "D) PASIVO CORRIENTE", level: 0, is_subtotal: true },
      { label: "3. Acreedores comerciales y otras cuentas a pagar", level: 1 },
      { label: "b) Proveedores a corto plazo", level: 2 },
    ],
  },
  // Cuadro moneda extranjera (BLOQUEADO: requiere importe en divisa origen).
  fx: {
    label: "Moneda extranjera (requiere divisa origen)",
    columns: [
      { id: "c-currency", label: "Moneda",                       col_type: "text" },
      { id: "c-eur",      label: "Importe en euros",             col_type: "value" },
      { id: "c-fx",       label: "Importe en moneda extranjera", col_type: "value" },
    ],
    rows: [
      { label: "Compras" },
      { label: "Ventas" },
...
const [sourceOverride, setSource]       = useState(null);
  ...
  // ── Consolidado: metadata de holdings/consolidaciones ──
  const [consolidations, setConsolidations] = useState(consolidationsProp);
  const [groupStructure, setGroupStructure] = useState(groupStructureProp);
  const [topParent, setTopParent] = useState("");
  useEffect(() => {
    if (!token) return;
    const h = { Authorization: `Bearer ${token}`, Accept: "application/json" };
const safeJson = (r) => (r.ok ? r.json() : { value: [] });
    const arrOf = (d) => d.value ?? (Array.isArray(d) ? d : []);
    Promise.all([
      fetch(`${BASE_URL}/v2/consolidations`,   { headers: h }).then(safeJson).then(arrOf).catch(() => []),
      fetch(`${BASE_URL}/v2/group-structure`,  { headers: h }).then(safeJson).then(arrOf).catch(() => []),
      fetch(`${BASE_URL}/v2/sources`,          { headers: h }).then(safeJson).then(arrOf).catch(() => []),
      fetch(`${BASE_URL}/v2/structures`,       { headers: h }).then(safeJson).then(arrOf).catch(() => []),
      fetch(`${BASE_URL}/v2/companies`,        { headers: h }).then(safeJson).then(arrOf).catch(() => []),
    ]).then(([cons, gs, src, str, co]) => {
      setConsolidations(cons); setGroupStructure(gs);
setSources(src); setStructures(str); setCompanies(co);
    });
  }, [token]);
  ...
  const { holdingOptions, rootParent } = useMemo(() => {
    ...
    const consolidatedGroups = new Set(
      ...
      ).map(c => c.GroupShortName ?? c.groupShortName).filter(Boolean)
    );
...
const topParentPickedRef = useRef(false);
  useEffect(() => {
    if (holdingOptions.length === 0) return;
    if (holdingOptions.some(h => h.value === topParent)) { topParentPickedRef.current = true; return; }
    const fallback = holdingOptions.find(h => h.value === rootParent) ? rootParent : (holdingOptions[0]?.value ?? "");
    queueMicrotask(() => setTopParent(fallback));
  }, [holdingOptions, rootParent, topParent]);

  // La "company" interna del guardado es el perímetro consolidado (prefijo para no
  // colisionar con empresas individuales). Todo el resto del código usa `company`.
  const company = topParent ? `__CONS__:${topParent}` : "";
...
setPrevRows(prev);
    ...
    return () => { cancelled = true; };
  }, [year, month, source, structure, topParent, fetchUploaded]);
...
col_periods: n._col_periods ?? {},
          ...
          // table_type se guarda para CUALQUIER epígrafe (no solo custom), para
          // persistir el tipo de tabla elegido al activarla manualmente.
...
// Periodos reanclados de esta nota → pivots del periodo elegido (para el export).
      ...
      const basePivot = (nRows.length && nCols.length)
        ? buildPivot({ note: n, rows: nRows, columns: nCols, sources: accountSources, overrides: ov, ccCodesByRow: ccByRowExp, intercoByCode, colPeriodSources: colPeriodSourcesExp })
        : new Map();
...
}, [notes, rowsByNote, colsByNote, accountSources, overridesByNote, applyCustomRows, resolveVariableValue, ccTagIndex, intercoByCode, sourcesForPeriod]);
 ...
 <PageHeader
        ...
        filters={[
          ...
          ...(holdingOptions.length > 0
            ? [{ label: "Holding", value: topParent, onChange: setTopParent, options: holdingOptions }]
            : []),
        ]}
