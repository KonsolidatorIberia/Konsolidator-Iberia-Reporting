...
const [expanded, setExpanded] = useState(false);
  ...
  useEffect(() => {
    ...
    // Expand-all/collapse-all resets openCount at parent — don't bump here.
...
const totalCols = 1 + visibleCompanies.length * (compareMode ? 4 : 1);
  // Whether this leaf has any underlying group accounts. Cheap scan — bails on
  // first match. Independent of expansion state so we can gate the toggle
  ...
  const hasAnyUnderlying = useMemo(() => {
    ...
    return false;
  }, [hasMappingChildren, uploadedData, groupToCf, node.code]);
...
expandAllVersion = 0, expandAllState = false,
  ...
  useEffect(() => {
    ...
    // Expand-all opens only the mapping structure — leaves that drill into
    // raw ERP accounts stay closed. Collapse-all still closes everything.
...
const totalCols = 1 + visibleCompanies.length * (compareMode ? 4 : 1);
  ...
  const hasAnyUnderlying = useMemo(() => {
    ...
    return false;
  }, [isSubtotal, uploadedData, groupToCf, node.AccountCode]);
  const isExpandable = hasAnyUnderlying;
...
const { access: resourceAccess } = useCurrentUserResourceAccess();
  ...
  const [consolidatedGroupStructure, setConsolidatedGroupStructure] = useState([]);

  // Fetch consolidations + group-structure for Perspective (holding) selector
  useEffect(() => {
    if (!token) return;
    const h = { Authorization: `Bearer ${token}`, Accept: "application/json" };
    Promise.all([
      fetch(`${BASE}/consolidations`,  { headers: h }).then(r => r.json()).then(d => d.value ?? d ?? []).catch(() => []),
      fetch(`${BASE}/group-structure`, { headers: h }).then(r => r.json()).then(d => d.value ?? d ?? []).catch(() => []),
    ]).then(([cons, gs]) => {
      setConsolidations(Array.isArray(cons) ? cons : []);
      setConsolidatedGroupStructure(Array.isArray(gs) ? gs : []);
    });
  }, [token]);
...
rowsArr.forEach(r => rows.set(String(r.account_code), {
        ...
        // uniformly across CUSTOM and legacy standards. If the legacy table
        // doesn't carry these columns, both fall through to "" harmlessly.
...
// CUSTOM wins; then sniffed built-in.
  ...
  // parent_code. Routes CUSTOM through the hierarchical MappedSheetRow renderer
  // (same as user mappings) instead of the flat SheetRow branch, so children
  // nest under their parents and only show when expanded.
  const customCfLiteral = useMemo(() => {
    ...
    // Roots whose section has no breaker (closing totals with section=null):
    // append with label null so they render flat at the end, no section bar.
    const looseNodes = [];
    ...
    if (looseNodes.length > 0) out.push({ label: null, color: null, nodes: looseNodes });
...
// Auto-reset state fields that fall outside the user's resource access set.
  // Uses functional updaters so the effect doesn't need to depend on the current
  // state value — no risk of loops, no eslint disables needed.
  useEffect(() => {
...
const visibleCompanies = useMemo(() => {
  ...
  // ─── Holding (Perspective) options ─────────────────────────────
  // Derived from group-structure: companies with children, or root parent,
  // narrowed to those present in a consolidation for the active period.
  const { holdingOptions, rootParent } = useMemo(() => {
    const gsRows = (consolidatedGroupStructure || []).map(g => ({
      company:   g.companyShortName ?? g.CompanyShortName ?? "",
    ...
    const consolidatedGroups = new Set(
      (consolidations || [])
        .filter(c =>
          String(c.Year ?? c.year) === String(year) &&
          String(c.Month ?? c.month) === String(month) &&
  ...
  }, [consolidatedGroupStructure, structure, consolidations, year, month, source, companies]);

  // Auto-pick a sensible Perspective when the holding list changes (done
  // at render time to avoid the set-state-in-effect lint).
  const holdingOptionsKey = holdingOptions.map(h => h.value).join("|");
  const [prevHoldingOptionsKey, setPrevHoldingOptionsKey] = useState(null);
  if (prevHoldingOptionsKey !== holdingOptionsKey) {
    setPrevHoldingOptionsKey(holdingOptionsKey);
    if (holdingOptions.length > 0 && !holdingOptions.some(h => h.value === topParent)) {
...
// Expand/collapse-all: bump version to re-sync every row to expandAllState
  ...
  // Count of rows currently expanded via manual click. Lets the toggle button
  // show the collapse icon when the user has opened rows individually even
  // though expandAllState is still false.
  ...
  const toggleExpandAll = () => {
    ...
    if (!next) setOpenCount(0);   // collapsing clears manual count
  };
// Search
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (let i = 0; i < maxAttempts; i++) {
        if (cancelled) return;
        const baseFilter = `Year eq ${tryY} and Month eq ${tryM} and Source eq '${source}' and GroupStructure eq '${structure}' and GroupShortName eq '${topParent}'`;
        try {
...
const [exporting, setExporting] = useState(false);
  ...
  // Pre-download modal state (mirrors Individual CF pattern)
...
// ── Cash flow mappings ────────────────────────────────────────────────
  ...
  // The system's hidden-override mapping, kept so "clear" restores the initial
  // default view instead of dropping to the raw standard.
...
// On load for a CUSTOM standard, pick the default mapping in priority order:
  //   1) a USER mapping flagged is_default for this company (from any origin —
  //      Scratch, template or custom; the mapping's own `standard` is irrelevant)
  //   2) the system hidden override (custom-no-user) as the fallback
  // The hidden override is always cached in hiddenOverrideRef so "reset to default"
  // can restore it, but a user default wins on first render.
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // A user mapping flagged default for this company wins, whatever its standard.
// The "standard" (default) CF mapping is a per-USER preference stored in
        // user_settings.preferences.cashflow_standard_mapping_id — the same value the
        // toggle in the mappings editor writes. If the user has one set, it wins over
        // the system hidden override.
      ...
      } catch (e) { console.warn("[CashFlowPage] default mapping load failed:", e); }
    })();
    return () => { cancelled = true; };
...
// Auto-reset cmpSource with functional updater — no state dep, no disables.
  useEffect(() => {
    if (!resourceAccess) return;
    setCmpSource(prev => {
      if (!prev || effectiveSources.length === 0) return prev;
...
// Fetch compare period when compare mode is active
  useEffect(() => {
    ...
    let cancelled = false;
    setCmpLoading(true);
    const baseFilter = `Year eq ${cmpYear} and Month eq ${cmpMonth} and Source eq '${cmpSource}' and GroupStructure eq '${structure}' and GroupShortName eq '${topParent}'`;
    const auth = { headers: { Authorization: `Bearer ${token}` } };
    Promise.all([
      fetch(`${BASE}/reports/consolidated-accounts?$filter=${encodeURIComponent(baseFilter)}`, auth).then(r => r.json()).then(d => d.value || []).catch(() => []),
...
rows.forEach(r => {
    ...
    return () => { cancelled = true; };
  }, [compareMode, cmpYear, cmpMonth, cmpSource, structure, topParent, token, groupToCf, cfMetadata]);
...
const writeNum = (rowN, colN, val, fillArgb, opts = {}) => {
      ...
      // For currency use rounded value to test empty; for percent use raw value
      // so tiny non-zero pcts (e.g. -0.03%) still render as "—" but -4.2% shows.
const isEmpty = val == null || !Number.isFinite(val) || (opts.percent
      ...
      if (isEmpty) {
        ...
        // Currency: round to integer so formulas (=A-B, =SUM) match what the
        // app displays. The app rounds A and B independently before subtracting,
        // so the export must too, otherwise deltas drift by ±1 unit.
        // Percent: keep raw value — needed for 0.1-decimal display (e.g. -4.2%).
const stored = opts.percent ? val : Math.round(val * 100) / 100;
        if (opts.formula) {
          cell.value = { formula: opts.formula, result: stored };
        } else {
          cell.value = stored;
        }
...
const buildDrillGroups = (cfCode) => {
      ...
      const ensure = (gc, gn) => {
        if (!byGroup.has(gc)) byGroup.set(gc, {
          ...
          coAmounts: {}, localRows: [], rowMap: new Map(), terminal: false,
        });
      ...
      filteredUploadedData.forEach(r => {
        ...
        if (!b.groupName && (r.AccountName ?? r.accountName)) b.groupName = String(r.AccountName ?? r.accountName);
        b.coAmounts[co] = (b.coAmounts[co] ?? 0) + amt;
        // Local row layer — matches Individual's drill. If the consolidated
        // data carries LocalAccountCode, we build one row per (local, company).
        const origin = r.Origin ?? r.origin ?? "";
        const lc = r.LocalAccountCode ?? r.localAccountCode ?? "";
        if (origin === "Journal" || !lc) return;
        const key = `ERP::${lc}::${co}`;
        if (!b.rowMap.has(key)) b.rowMap.set(key, {
          localCode: lc, localName: r.LocalAccountName ?? r.localAccountName ?? "",
      ...
      (journalEntries || []).forEach(j => {
        ...
        const key = `JRN::${jn}::${co}`;
        if (!b.rowMap.has(key)) b.rowMap.set(key, {
          localCode: jn || "JRN", localName: jh || jt || "Journal",
...
writeNum(curRow, startCol + 2, delta,    "FFF5F7FF", {
          ...
          writeNum(curRow, startCol + 3, deltaPct, "FFF0F3FF", {
            ...
            formula: `IF(ABS(N(${bRef}))<0.0001,NA(),(N(${aRef})-N(${bRef}))/ABS(N(${bRef}))*100)`,
...
const row = ws.getRow(curRow);
      ...
      // Only apply outline collapse when drilldown is enabled. Without drilldown
      // we want a flat brief report — no collapsed skeleton.
      if (drilldown) {
...
exportCfLiteral.forEach(section => {
    ...
    // compare each company is a single column. Total column (if shown) also gets a divider.
    {
      const subCols = sheetCompare ? 4 : 1;
      const borderStyle = { style: 'medium', color: { argb: 'FF6B7280' } };
      const boundaries = [];
      for (let i = 0; i < visCo.length; i++) {
        boundaries.push(2 + i * subCols);
      }
      if (showTotals) boundaries.push(2 + visCo.length * subCols);
...
const b = getCmpValAt(code, co);
            ...
            const dColor = (delta == null || Math.round(delta * 100) === 0) ? GRAY : (delta > 0 ? GRN : RED);
...
const cmpV = cmpMap?.get(`GRP::${groupCode}::${co}`) ?? 0;
            const delta = v - cmpV;
            const pct = Math.abs(cmpV) > 1e-9 ? ((v - cmpV) / Math.abs(cmpV)) * 100 : null;
            const dColor = (Math.round(delta * 100) === 0) ? GRAY : (delta > 0 ? GRN : RED);
...
const pdfCfLiteral = activeMapping?.cfLiteral ?? customCfLiteral;
      if (pdfCfLiteral) {
        // Sum nodes need descendant rollup (matches MappedSheetRow.getContrib).
        // pushAccountRow uses getValAt (flat) which is 0 for sums whose postings
        // live on descendants — pushSumRow aggregates leaves so values match the app.
        const collectLeafCodes = (n, acc = []) => {
          if (!n.children || n.children.length === 0) {
            acc.push(n.code);
          } else {
...
const ra = Math.round(a * 100) / 100;
              ...
              const delta = ra - rb;
              const pct = Math.abs(rb) > 1e-9 ? ((ra - rb) / Math.abs(rb)) * 100 : null;
              const dColor = (delta == null || Math.round(delta * 100) === 0) ? GRAY : (delta > 0 ? GRN : RED);
cells.push({ content: fmt(ra),     styles: { fontStyle: "bold", textColor: ra < 0 ? RED : TEXTDK } });
              cells.push({ content: fmt(rb),     styles: { fontStyle: "bold", textColor: REDDK } });
      ...
      doc.setTextColor(...GRAY);
{
        ...
        doc.text(`${T("file_generated")} ${stamp}`, W - 8, 22, { align: "right" });
        if (exportedByName) doc.text(`${T("file_exported_by", "por")} ${exportedByName}`, W - 8, 25, { align: "right" });
      }
...
...(effectiveSources.length > 0
          ...
          ...(holdingOptions.length > 0
            ? [{ label: T("filter_perspective", "Perspective"), value: topParent, onChange: setTopParent, options: holdingOptions }]
            : []),
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {reportMappings.map(m => {
                      ...
                      return (
                        <button key={m.mapping_id}
onClick={() => {
                            // El API de reporte no tiene getMapping; la fila del listado
                            // ya trae cf_tree (select=*), la aplicamos directamente.
